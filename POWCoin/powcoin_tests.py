...
def test_block_extending_fork():
    ...
    mine_block(node, ids.alice_public_key, node.blocks[0], [])
    # Alice mines block on top of her branch
    mine_block(node, ids.alice_public_key, node.branches[0][0], [])
...
def test_block_forking_fork():
    ...
    third = mine_block(
        node, ids.alice_public_key, node.branches[0][0], [], nonce=second.nonce + 1
    )
...
def test_successful_reorg():
    ...
    b2 = mine_block(node, ids.bob_public_key, node.blocks[1], [bob_to_alice])
    ...
    alice_to_bob = send_tx(alice_node, ids.alice_private_key, ids.bob_public_key, 20)
    a3 = mine_block(node, ids.alice_public_key, node.branches[0][0], [alice_to_bob])
...
def test_unsuccessful_reorg():
    ...
    # during reorg
    mine_block(node, ids.alice_public_key, node.branches[0][0], [alice_to_bob])
